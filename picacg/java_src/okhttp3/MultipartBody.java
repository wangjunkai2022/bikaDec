        ...
        for (int i = 0; i < size; i++) {
            ...
            if (headers != null) {
                ...
                for (int i2 = 0; i2 < size2; i2++) {
                    dVar.aV(headers.name(i2)).k(COLONSPACE).aV(headers.value(i2)).k(CRLF);
                }
            }
            MediaType contentType = requestBody.contentType();
            if (contentType != null) {
                dVar.aV("Content-Type: ").aV(contentType.toString()).k(CRLF);
            }
            long contentLength = requestBody.contentLength();
            if (contentLength != -1) {
                dVar.aV("Content-Length: ").y(contentLength).k(CRLF);
            } else if (z) {
                cVar.clear();
                return -1L;
            }
